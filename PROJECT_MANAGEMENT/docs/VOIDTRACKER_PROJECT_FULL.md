PROJECT: OMNI-NEXUS (The Infinite Logistics Engine)
...
I. THE CORE STRATEGY: "THE 24-HOUR LOGISTICS LOOP"

Most competitors do A-to-B delivery. ALEET owns the entire lifecycle of the asset within the client's ecosystem.
The Value Proposition:
* For the HQ (Master Profile): Total visibility of inventory moving between their locations, not just to them.
* For the Branch (Recipient): Work starts faster. Parts are there when they open (Night), and returns are cleared instantly.


II. STRATEGY PILLAR 1: THE "GHOST" NIGHT DELIVERY (Unattended)

Market Gap: Dealerships and Hotels lose 3-4 hours every morning waiting for deliveries. Your Solution: "Zero-Contact, Zero-Trust" Architecture.
* The Workflow Strategy:
    1. The "Keyless" Handshake: The driver does not ring a bell. The Instruction Tile provides the Gate Code/Keybox Code.
    2. The "Anchor" Scan (Crucial):
        * We place a permanent, weather-resistant QR code on the wall of the client's "Safe Zone" (Cage/Garage).
        * Rule: The delivery cannot be marked complete unless that specific wall code is scanned. GPS is not enough (GPS drifts). The Scan is absolute proof of entry.
    3. The "Evidence Chain":
        * Driver must take a photo of the goods inside the cage.
        * AI Audit: n8n sends the photo to a vision model. If the cage door is open in the photo -> ALERT.
* B2B Monetization:
    * Charge a "Night Premium". Clients pay it happily because their mechanics can start billing hours at 8:00 AM sharp using the parts you delivered at 4:00 AM.


III. STRATEGY PILLAR 2: THE "WHITE GLOVE" DAY DELIVERY (Attended)

Market Gap: Standard couriers just drop boxes. They don't handle COD (Cash on Delivery) or sensitive documents well. Your Solution: "The Concierge Protocol."
* The Workflow Strategy:
    1. Dynamic Identity Check:
        * System prompts driver: "Ask for Name."
        * If the recipient is not on the "Authorized Staff List" (managed in Admin), the driver cannot release the package.
    2. Financial Collection (COD):
        * If the shipment is "COD," the PWA turns into a POS terminal (displays QR for payment/instructions for cash).
        * The signature is blocked until the "Payment Collected" toggle is active.
* B2B Monetization:
    * "Value-Added Services" (VAS): Charge extra for "Identity Verification," "COD Collection," or "Unpacking/Debris Removal."


IV. STRATEGY PILLAR 3: THE "GATEKEEPER" RETURNS (Reverse Logistics)

Market Gap: Returns are the biggest money pit. Drivers pick up items that are broken, leaking, or incomplete, and the courier company gets blamed. Your Solution: "Validation at the Edge."
* The Workflow Strategy:
    1. Pre-Authorization: The Branch (Profile 2) creates a "Return Request" in the Portal.
    2. The "Inspector" Driver:
        * When the driver arrives, the PWA does not say "Pick Up." It says "Inspect."
        * Scenario: Asset = "Defective Battery."
        * Instruction: "Is the casing cracked?" [Yes/No]. "Is it in a plastic bag?" [Yes/No].
    3. The Veto:
        * Logic: If "Casing Cracked" = YES -> STOP.
        * Result: Driver refuses pickup. System logs it. Client HQ is notified: "Pickup refused - Safety Hazard."
* B2B Value: You save the client money by preventing "Trash" from entering the supply chain. You become their Quality Control.


V. STRATEGY PILLAR 4: THE DEALER-TO-DEALER TRANSFER (Horizontal Logistics)

Market Gap: Dealer A has the part. Dealer B needs it. They usually call a taxi or send a junior employee (expensive/risky). Your Solution: "The Internal Marketplace."
* The Workflow Strategy:
    1. Peer-to-Peer Booking:
        * Dealer A logs in and selects "Ship to Dealer B" (Dropdown).
        * No address entry needed (Address is hardcoded in Dealer B's profile).
    2. Cost Allocation (The "Mad" Finance Feature):
        * The System automatically Debits Dealer B's cost center and Credits Dealer A's inventory log.
        * The Invoice shows a "Transfer Fee" payable to YOU (The Carrier).
    3. The "Milk Run" Optimization:
        * Your algorithms see that a driver is dropping off at Dealer A and needs to go to Dealer B anyway.
        * The transfer becomes almost pure profit because the truck is already moving that direction.


VI. THE "DYNAMIC PRICING" REVENUE MODEL

To secure the Enterprise B2B contract, you move away from simple "Per Km" pricing. You implement Logic-Based Billing.
1. The Base Subscription (SaaS):
    * Monthly fee for the "Customer Portal" and "Admin Access."
    * Grants access to the dashboard, reporting, and API.
2. The "Matrix" Activity Fee:
    * Zone A to Zone A: €10.
    * Zone A to Zone B: €25.
    * Unit Surcharge:
        * Type: Envelope = +€0.
        * Type: Scooter = +€15.
        * Type: Dirty Towels = +€5 (Handling Fee).
3. The "SLA" Upsell:
    * Standard (Day): Delivered by 5 PM.
    * Priority (Transfer): Delivered in 4 hours (+50%).
    * Secure (Night): Delivered between 2 AM - 6 AM with Photo Audit (+30%).


VII. EXECUTION: HOW n8n POWERS THIS STRATEGY

* For Night Deliveries:
    * n8n Trigger: Shipment_Scanned_At_Wall.
    * Action: Immediately email the "Night Report" to the Store Manager so they see it first thing in the morning. "Your parts are in Cage B. Here is the photo."
* For Transfers:
    * n8n Trigger: Transfer_Booked.
    * Action: Check Dealer A's hours. If "Closed", auto-schedule pickup for next morning. Send confirmation to Dealer B: "Part coming tomorrow."
* For Returns:
    * n8n Trigger: Return_Refused (Driver clicked "Damaged").
    * Action: Create ticket in Client's ERP System (SAP/Salesforce) -> "Warranty Claim Rejected - Physical Damage."
...
* Live Operations Map (Real-time view of all active drivers/shipments)
* SLA Monitor (Widgets: "At Risk," "Breached," "On Time")
* Financial Pulse (Today's Revenue vs. Cost Accruals)
* System Health (n8n Webhook Latency, API Uptime)

2. 🏢 TENANT & ENTITY MANAGER (The "Who")

* Client Management
    * [Create New Client]
    * Client List (Search by Name/ID)
        * Client Detail View:
            * Global Settings: Logo, Brand Colors (Hex), Unit System (Metric/Imperial).
            * Hierarchy Tree: Master Profile -> Departments -> Cost Centers.
            * Feature Toggles: [x] Enable Transfers, [ ] Enable Returns, [x] Enable COD.
* User & RBAC Config
    * Role Definitions (e.g., "Warehouse Manager," "Junior Dispatcher").
        * Permission Matrix: Checkboxes for Can_View_Finance, Can_Book_Shipment, Can_Edit_Address.
    * Department Mapping: Assign Users to specific Departments (Data Scoping).

3. 📦 ASSET & WORKFLOW ENGINE (The "What" & "How")

* Asset Class Definitions (The "Pokemon vs. Towel" Creator)
    * [Create New Asset Type]
    * Field Builder:
        * Label: "Dirty Towel"
        * Icon: [Upload SVG]
        * Custom Fields:
            * weight_kg (Type: Number, Min: 1, Max: 50)
            * is_wet (Type: Boolean)
            * photo_required (Type: Boolean, Default: True)
* Workflow / Journey Builder (State Machine)
    * Delivery Types: "Night Drop," "Day Standard," "Dealer Transfer."
    * Step Config:
        * Step 1: Scan_At_Pickup (Validation: GPS < 50m).
        * Step 2: Transit (Passive GPS tracking).
        * Step 3: Arrival_Scan (Validation: Wall QR Code).
        * Step 4: Proof_Of_Delivery (Photo + Signature).

4. 💰 FINANCE & PRICING MATRIX (The "Why")

* Rate Cards (Dynamic Pricing)
    * Zone Manager: Draw Polygons on Map (Zone A, Zone B).
    * Matrix Config:
        * Row: Source Zone | Column: Dest Zone.
        * Cell Logic: Base_Price + (Weight * 0.50 EUR).
    * Surcharges:
        * Rule: IF Time > 22:00 AND Time < 06:00 THEN Add +20%.
        * Rule: IF Asset_Type = "Hazardous" THEN Add +50 EUR.
* Invoicing Engine
    * Billing Cycles: Weekly / Monthly / Custom.
    * Template Designer: HTML/CSS Editor for PDF Invoice.
    * Cost Allocation: Enable "Split by Department" (Yes/No).

5. 🤖 AUTOMATION & INTEGRATION (The "Brain")

* n8n Webhook Registry
    * Event Triggers:
        * shipment.created -> [Input n8n URL]
        * shipment.delivered -> [Input n8n URL]
        * driver.anomaly_alert -> [Input n8n URL]
        * invoice.dispute -> [Input n8n URL]
    * Payload Tester: Button to send "Mock JSON" to n8n to test flows.
* Driver App (PWA) Settings
    * Magic Link Expiry: (e.g., 12 Hours).
    * Geofence Tolerance: (e.g., 300 meters).
    * Scanner Type: [x] QR Code, [x] Barcode 128, [ ] DataMatrix.

6. 🛡️ AUDIT & SECURITY (The "Truth")

* Global Event Log: Immutable list of every click, scan, and API call.
* Dispute Center: View flagged shipments & resolved billing issues.
...
Technical Stack: PostgreSQL (Primary DB). Why: We need strict relational integrity for Billing & Users (SQL), but infinite flexibility for Cargo Attributes (NoSQL/JSONB). PostgreSQL handles both natively.
...
* asset_definitions: The catalog of things we can ship.
    ...
    * JSONB Column form_schema: This is the magic. It defines the inputs.JSON[
...
We do not build everything at once. We build in layers of complexity.

PHASE 1: THE SKELETON (MVP) – Weeks 1-6

Goal: A driver can move a box from A to B using a link, and an Admin can see it.
1. Core Engine: Setup PostgreSQL + Node.js (NestJS or Fastify recommended for speed).
2. Admin v1: Simple CRUD to create Users and Shipments.
3. Driver PWA v0.5:
    * Route View (List of addresses).
    * "Magic Link" Auth.
    * Deep-link to Google Maps.
    * Basic Status Buttons ("Arrived", "Done").
...
1. Scanner Integration: Implement html5-qrcode in the PWA.
2. Geofence Logic: Implement the "300m Alert" logic in the backend.
3. The "Meta-Schema" implementation: Build the Dynamic Form renderer in the Frontend.
    * Test: Create a "Pizza" asset type and a "Gold Bar" asset type in Admin. Ensure Driver sees different fields for each.
4. n8n Connection: Setup the Webhook Emitter. Connect the first "Slack Alert" workflow.
...
1. Invoicing Module:
    ...
    * PDF Generator integration (puppeteer or external API).
2. The "Tile" Dashboard: Build the drag-and-drop Admin UI.
3. Rate Card Matrix: Implement the logic to calculate costs based on the JSONB attributes (e.g., IF attributes.weight > 50 THEN price + 10).
...
1. The Codebase: A Mono-repo (Frontend + Backend).
2. The Admin Configurator: A "No-Code" environment for Logistics Managers.
3. The PWA: A lightweight, disposable, scanning-capable web interface.
4. The n8n Blueprint: A library of JSON workflows for Alerts, Billing, and Notifications.
This is Part 4 of the Project Master File.
You asked to go "Full Mad AI." This section moves beyond standard logistics into Cognitive Operations. We are designing a system that doesn't just record what happened—it anticipates, guides, and talks.
Here are the "Moonshot" features that will make this system a market unicorn.
